 ...
 * 
 * Note from Ms. Smith
 * - this is sample code from the online text, modified a bit for our purposes
 * - the only code to write is in the doFrame method, all other code handles the user interface
 * - colors can be changed if you wish
 * - Algorithm - IMPORTANT - start without the world wrapping around, add that after the
 *       basic algorithm for cell generation is complete.  Rules are given above.
 * - Wikipedia (and MANY other sites) has examples of interesting known patterns - 
        ...
        for (int r = 0; r < GRID_SIZE; r++) {
            for (int c = 0; c < GRID_SIZE; c++) {
                if (alive[r][c])
                    ...
                    display.setColor(r,c,Color.GRAY);  // Shows the background color, black.
            }
        }
